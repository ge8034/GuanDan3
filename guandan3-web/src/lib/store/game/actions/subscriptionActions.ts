...
import {
  ...
  computeLastActionFromRecentTurns,
...
export function subscribeGame(
  ...
  const ensureTurnsChannel = (nextGameId: string | null) => {
    ...
    if (turnsChannel) {
      ...
      turnsChannel = null;
      turnsGameId = null;
      realtimeOptimizer.updateConnectionStatus(
        turnsConnectionId,
        'disconnected'
      );
    }
    ...
    turnsChannel = supabase
      ...
      .on(
        'postgres_changes',
        {
          ...
          schema: 'public',
          table: 'turns',
          filter: `game_id=eq.${nextGameId}`,
        },
        async (payload) => {
          const startTime = performance.now();
          const newTurn = payload.new as Record<string, unknown>;

          // 检查原始 turn_no 是否为有效数字
          const isValidTurnNo = typeof newTurn.turn_no === 'number';

          const incoming: TurnRow = {
            turn_no: isValidTurnNo ? (newTurn.turn_no as number) : 0,
          ...
          if (isValidTurnNo) {
            ...
            state.setGame({
              ...
              lastAction: computeLastActionFromRecentTurns(
                nextRecent,
                state.currentSeat
              ),
            });
  ...
  const gameChannel = supabase
    ...
    .on(
      ...
      async (payload) => {
        ...
        const statePublic = newGame.state_public as
        ...
        if (
          newGame.id &&
          previousGameId === newGame.id &&
          typeof previousTurnNo === 'number'
        ) {
          const gap = (newGame.turn_no as number) - previousTurnNo;
          if (gap > 1) {
            try {
              const turns = await state.fetchTurnsSince(
                String(newGame.id),
                previousTurnNo
              );
