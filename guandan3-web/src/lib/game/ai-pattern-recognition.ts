...
export function findSequencePairs(cards: Card[], levelRank: number): Card[][] {
  ...
  for (let i = 0; i < sortedValues.length - 2; i++) {
    ...
    if (sequenceValues[2] - sequenceValues[0] === 2) {
      const perValuePairs = sequenceValues.map(v => generateCombinations(valueMap.get(v) || [], 2))
...
export function findSequenceTriples(cards: Card[], levelRank: number): Card[][] {
...
export function findQuadWithTwo(cards: Card[], levelRank: number): Card[][] {
  ...
  quads.forEach(quad => {
    const remainingCards = cards.filter(card => 
      getCardValue(card, levelRank) !== quad.value
    )
...
function generateCartesianProduct<T>(lists: T[][]): T[][] {
  ...
  return lists.reduce<T[][]>((acc, list) => acc.flatMap(a => list.map(item => [...a, item])), [[]])
}
