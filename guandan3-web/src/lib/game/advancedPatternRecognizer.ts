...
export class AdvancedPatternRecognizer {
  ...
  analyzeCards(cards: Card[]): PatternAnalysisResult {
    ...
    // 逢人配组合识别
    const wildCardCombinations = this.generateWildCardCombinations(
      cards,
      wildCards
    );
    ...
    return {
      ...
      alternativePatterns: patterns.filter(
        (p) => p !== this.selectBestPattern(patterns)
      ),
  ...
  private recognizeBasicPatterns(
    cards: Card[],
    wildCards: Card[]
  ): AdvancedPattern[] {
    const patterns: AdvancedPattern[] = [];
    const values = cards
      .map((c) => getCardValue(c, this.levelRank))
      .sort((a, b) => a - b);
  ...
  private recognizeAdvancedPatterns(
    cards: Card[],
    wildCards: Card[]
  ): AdvancedPattern[] {
    const patterns: AdvancedPattern[] = [];
  ...
  private recognizeBombs(cards: Card[], wildCards: Card[]): AdvancedPattern[] {
    ...
    if (cards.length === 6 && !hasJoker) {
      ...
      if (quadVals.length === 1) {
        ...
        if (
          remainingVals.length === 2 ||
          (remainingVals.length === 1 && counts[remainingVals[0]] === 2)
        ) {
          patterns.push({
            type: 'bombWithTwo',
            cards,
            ...
            secondaryValue:
              remainingVals.length === 1
                ? remainingVals[0]
                : Math.max(...remainingVals),
            complexity: 6,
            isWildCardUsed: false,
            wildCards: [],
          });
  ...
  findBestMove(
    cards: Card[],
    targetPattern?: { type: string; primaryValue: number; cardCount: number }
  ): AdvancedPattern | null {
    ...
    if (validPatterns.length === 0) {
      // 检查炸弹
      const bombPatterns = analysis.patterns.filter(
        (p) => p.type === 'bomb' || p.type === 'rocket'
      );
      if (bombPatterns.length > 0) {
        return this.selectBestPattern(bombPatterns);
      }
