...
describe('Advanced Pattern Recognizer', () => {
  ...
  describe('Basic Pattern Recognition', () => {
    ...
    it('should recognize pair', () => {
      const recognizer = new AdvancedPatternRecognizer(2);
      const cards = [createCard('S', '3', 3), createCard('H', '3', 3)];
      const result = recognizer.analyzeCards(cards);
  ...
  describe('Advanced Pattern Recognition', () => {
    ...
    it('should recognize straight (顺子)', () => {
      ...
      const straightPattern = result.patterns.find(
        (p) => p.type === 'straight'
      );
    ...
    it('should recognize sequence pairs (连对)', () => {
      ...
      const seqPairsPattern = result.patterns.find(
        (p) => p.type === 'sequencePairs'
      );
    ...
    it('should recognize sequence triples (飞机)', () => {
      ...
      const seqTriplesPattern = result.patterns.find(
        (p) => p.type === 'sequenceTriples'
      );
  ...
  describe('Bomb Recognition', () => {
    ...
    it('should recognize bomb with two (四带二)', () => {
      ...
      const bombWithTwoPattern = result.patterns.find(
        (p) => p.type === 'bombWithTwo'
      );
  ...
  describe('Pattern Strength Calculation', () => {
    it('should calculate higher strength for bombs', () => {
      ...
      const bombStrength = recognizer.calculatePatternStrength(
        bombAnalysis.bestPattern!
      );
      const singleStrength = recognizer.calculatePatternStrength(
        singleAnalysis.bestPattern!
      );
    ...
    it('should calculate higher strength for rocket than regular bomb', () => {
      ...
      const rocketStrength = recognizer.calculatePatternStrength(
        rocketAnalysis.bestPattern!
      );
      const bombStrength = recognizer.calculatePatternStrength(
        bombAnalysis.bestPattern!
      );
  ...
  describe('Best Move Selection', () => {
    ...
    it('should find move that beats target pattern', () => {
      ...
      expect(bestMove?.primaryValue).toBeGreaterThan(
        targetPattern.primaryValue
      );
    });
