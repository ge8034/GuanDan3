...
describe('ai.decideMove', () => {
  ...
  it('跟单张时选择能压过的最小单张', () => {
    ...
    const move = decideMove(hand, [c({ id: 3, suit: 'C', rank: '5', val: 5 })], 2, 'hard', false)
    expect(move.type).toBe('play')
    expect(move.cards?.map(x => x.id)).toEqual([2])
  })
  ...
  it('无法跟牌时优先用炸弹压制非炸弹', () => {
    ...
    const move = decideMove(hand, [c({ id: 9, suit: 'C', rank: 'A', val: 14 })], 2, 'hard', false)
    expect(move.type).toBe('play')
  ...
  it('跟炸弹时可用更大张数的炸弹压制', () => {
    ...
    const move = decideMove(hand, [c({ id: 9, suit: 'S', rank: '7', val: 7 }), c({ id: 10, suit: 'D', rank: '7', val: 7 }), c({ id: 11, suit: 'C', rank: '7', val: 7 }), c({ id: 12, suit: 'H', rank: '7', val: 7 })], 2, 'hard', false)
    expect(move.type).toBe('play')
  ...
  it('跟对子时选择能压过的最小对子，否则pass', () => {
    ...
    const ok = decideMove(hand, [c({ id: 9, suit: 'S', rank: '5', val: 5 }), c({ id: 10, suit: 'D', rank: '5', val: 5 })], 2, 'hard', false)
    expect(ok.type).toBe('play')
    expect(ok.cards?.length).toBe(2)

    const no = decideMove(hand, [c({ id: 11, suit: 'S', rank: '7', val: 7 }), c({ id: 12, suit: 'D', rank: '7', val: 7 })], 2, 'hard', false)
  ...
  it('跟三张时选择能压过的最小三张，否则pass', () => {
    ...
    const ok = decideMove(hand, [c({ id: 9, suit: 'S', rank: '5', val: 5 }), c({ id: 10, suit: 'D', rank: '5', val: 5 }), c({ id: 11, suit: 'C', rank: '5', val: 5 })], 2, 'hard', false)
    expect(ok.type).toBe('play')
    expect(ok.cards?.length).toBe(3)

    const no = decideMove(hand, [c({ id: 12, suit: 'S', rank: '7', val: 7 }), c({ id: 13, suit: 'D', rank: '7', val: 7 }), c({ id: 14, suit: 'C', rank: '7', val: 7 })], 2, 'hard', false)
    expect(no.type).toBe('pass')
  })
  ...
  it('跟炸弹时同张数可比较点数，否则pass', () => {
    ...
    const ok = decideMove(hand, [c({ id: 9, suit: 'S', rank: '8', val: 8 }), c({ id: 10, suit: 'D', rank: '8', val: 8 }), c({ id: 11, suit: 'C', rank: '8', val: 8 }), c({ id: 12, suit: 'H', rank: '8', val: 8 })], 2, 'hard', false)
    expect(ok.type).toBe('play')
    expect(ok.cards?.length).toBe(4)

    const no = decideMove(hand, [c({ id: 13, suit: 'S', rank: '10', val: 10 }), c({ id: 14, suit: 'D', rank: '10', val: 10 }), c({ id: 15, suit: 'C', rank: '10', val: 10 }), c({ id: 16, suit: 'H', rank: '10', val: 10 })], 2, 'hard', false)
    expect(no.type).toBe('pass')
  })
