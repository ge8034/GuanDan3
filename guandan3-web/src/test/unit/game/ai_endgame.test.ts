...
describe('AI Strategy - Guard & Difficulty', () => {
  ...
  it('needsSupport=true 且 lastPlay=null 时不会崩溃', () => {
    ...
    const teammateSituation = { isLeading: true, isStrong: false, needsSupport: true, canLead: true }

    const move = findOptimalMove(hand, null, 2, 'hard', true, teammateSituation)
  ...
  it('recordDecisionMetrics 超过上限会裁剪并可计算胜率', () => {
    ...
    for (let i = 0; i < 20; i++) {
      recordDecisionMetrics('follow', 'single', 10, 'hard', 10, Date.now() - 3000)
    }
  ...
  it('evaluateMove 可覆盖不同牌型与难度分支', () => {
    const quadWithTwo: Card[] = [
      c({ id: 1, suit: 'S', rank: '7', val: 7 }),
      c({ id: 2, suit: 'D', rank: '7', val: 7 }),
      c({ id: 3, suit: 'C', rank: '7', val: 7 }),
      c({ id: 4, suit: 'H', rank: '7', val: 7 }),
      c({ id: 5, suit: 'S', rank: '9', val: 9 }),
      c({ id: 6, suit: 'D', rank: '10', val: 10 }),
    ]

    const hand = [...quadWithTwo]
    const hard = evaluateMove({ type: 'play', cards: quadWithTwo }, hand, null, 2, 'hard', true)
    const medium = evaluateMove({ type: 'play', cards: quadWithTwo }, hand, null, 2, 'medium', true)
    const easy = evaluateMove({ type: 'play', cards: quadWithTwo }, hand, null, 2, 'easy', true)
    ...
    expect(hard.reasoning).toContain('Bomb play')
    expect(hard.reasoning).toContain('Quad with two')
    expect(hard.reasoning).toContain('Leading play')
    expect(hard.reasoning).toContain('Near end of hand')

    const airplaneWithWings: Card[] = [
      c({ id: 10, suit: 'S', rank: '3', val: 3 }),
      c({ id: 11, suit: 'D', rank: '3', val: 3 }),
      c({ id: 12, suit: 'C', rank: '3', val: 3 }),
      c({ id: 13, suit: 'S', rank: '4', val: 4 }),
      c({ id: 14, suit: 'D', rank: '4', val: 4 }),
      c({ id: 15, suit: 'C', rank: '4', val: 4 }),
      c({ id: 16, suit: 'S', rank: '7', val: 7 }),
      c({ id: 17, suit: 'D', rank: '8', val: 8 }),
    ]

    const wingEval = evaluateMove({ type: 'play', cards: airplaneWithWings }, airplaneWithWings, null, 2, 'hard', false)
    expect(wingEval.reasoning).toContain('Sequence triple with wing')
  })
  ...
  it('findBestSupportMove 在无法支援时返回 pass', () => {
    ...
    const teammateSituation = { isLeading: false, isStrong: false, needsSupport: true, canLead: true }

    const move = findBestSupportMove(hand, lastPlay, 2, teammateSituation)
    expect(move.type).toBe('pass')
  })

  it('findBestSupportMove 可用飞机带翅膀支援', () => {
    const hand: Card[] = [
    ...
    expect(move.type).toBe('play')
    expect(move.cards?.length).toBe(8)
  })
  ...
  it('needsSupport=true 但支援失败时仍可走常规最优出牌', () => {
    ...
    const move = findOptimalMove(hand, lastPlay, 2, 'hard', false, teammateSituation)
    expect(move.type).toBe('play')
