...
// Helper function to create complete query chain mock
...
describe('useGameStore基本操作', () => {
  ...
  it('updateHand会更新手牌', () => {
    useGameStore
      .getState()
      .updateHand([makeCard({ id: 1 }), makeCard({ id: 2 })]);
...
describe('useGameStore.submitTurn', () => {
  ...
  it('提交失败会回滚乐观更新', async () => {
    ...
    (supabase.rpc as any).mockResolvedValue({
      ...
      error: {
        code: 'P0001',
        message: 'not_your_turn: You are Seat 1, Current is Seat 0',
      },
    });
  ...
  it('生产环境下未知错误会记录简要错误日志', async () => {
    ...
    await withNodeEnv('production', async () => {
      ...
      expect(
        (errSpy as any).mock.calls.some((c: any[]) =>
          String(c[0]).includes('Submit turn failed:')
        )
      ).toBe(true);
      expect(
        (errSpy as any).mock.calls.some((c: any[]) =>
          String(c[0]).includes('Submit turn failed detailed:')
        )
      ).toBe(false);
    });
    errSpy.mockRestore();
...
describe('useGameStore.fetchGame', () => {
      ...
      recentTurns: [
        {
          turn_no: 1,
          seat_no: 1,
          payload: { type: 'play', cards: [makeCard({ id: 98 })] },
        },
      ],
    });
    const gameQuery = {
  ...
  it('hands查询错误时不会更新myHand', async () => {
    ...
    // room_members 查询返回有效座位号，让 game_hands 查询被执行
    const memberQuery = {
      select: vi.fn().mockReturnThis(),
      eq: vi.fn().mockReturnThis(),
      maybeSingle: vi.fn().mockResolvedValue({
    ...
    const handQuery = {
      ...
      maybeSingle: vi
        .fn()
        .mockResolvedValue({ data: null, error: new Error('hands error') }),
    };
    // turns 查询（fetchLastTrickPlay）
    const turnsQuery = {
      select: vi.fn().mockReturnThis(),
...
describe('useGameStore.fetchLastTrickPlay', () => {
  ...
  it('无gameId返回null', async () => {
    useGameStore.setState({ gameId: null });
    await expect(
      useGameStore.getState().fetchLastTrickPlay()
    ).resolves.toBeNull();
  });
    ...
    const turnsQuery = {
      ...
      limit: vi.fn().mockResolvedValue({
        data: [
          ...
          { payload: { type: 'pass' }, seat_no: 2, turn_no: 2 },
          {
            payload: { type: 'play', cards: [makeCard({ id: 5 })] },
            seat_no: 3,
            turn_no: 1,
          },
        ],
...
describe('useGameStore.fetchTurnsSince', () => {
  ...
  it('PGRST202时返回空数组', async () => {
    ...
    await expect(
      useGameStore.getState().fetchTurnsSince('g-1', 0)
    ).resolves.toEqual([]);
  });
  ...
  it('成功时返回turns数组', async () => {
    ...
    await expect(
      useGameStore.getState().fetchTurnsSince('g-1', 0)
    ).resolves.toEqual([{ turn_no: 1, seat_no: 0, payload: { type: 'pass' } }]);
  });
  ...
  it('同一game下增量补拉会合并recentTurns并更新lastAction', async () => {
    ...
    expect(useGameStore.getState().recentTurns.map((t) => t.turn_no)).toEqual([
      3, 2,
    ]);
...
describe('useGameStore.subscribeGame', () => {
  ...
  it('存在gameId时会同时订阅turns并在取消时移除两个channel', () => {
    ...
    (supabase.channel as any)
      .mockReturnValueOnce(gamesChannel)
      .mockReturnValueOnce(turnsChannel);
  ...
  it('turn跳跃时会触发增量补拉并刷新lastAction', async () => {
    ...
    const fetchLastTrickPlay = vi.fn().mockResolvedValue({
      type: 'play',
      seatNo: 1,
      cards: [makeCard({ id: 1 })],
    });
  ...
  it('rankings增长时会触发fetchGame强制刷新', async () => {
    ...
    await handlers[0]({
      ...
      new: {
        ...
        turn_no: 1,
        current_seat: 1,
        state_public: { counts: [27, 27, 27, 27], rankings: [0] },
      },
  ...
  it('turns插入事件会在当前game下刷新lastAction', async () => {
    ...
    await turnsHandlers[0]({
      new: {
        turn_no: 1,
        seat_no: 2,
        payload: { type: 'play', cards: [makeCard({ id: 9 })] },
      },
    });
    expect(useGameStore.getState().lastAction).toEqual({
      type: 'play',
  ...
  it('turns插入事件在turn_no非数字时会fallback到fetchLastTrickPlay', async () => {
    ...
    const unsub = useGameStore.getState().subscribeGame('room-1');
    await turnsHandlers[0]({
      new: { turn_no: '1', seat_no: 2, payload: { type: 'pass' } },
    });
  ...
  it('turns插入连续三次pass后lastAction会变为null', async () => {
    ...
    const unsub = useGameStore.getState().subscribeGame('room-1');
    await turnsHandlers[0]({
      new: { turn_no: 4, seat_no: 0, payload: { type: 'pass' } },
    });
    expect(useGameStore.getState().lastAction).toBeNull();
    unsub();
  });
  ...
  it('gameId变化时会切换turns订阅并重置recentTurns与lastAction', async () => {
    ...
    (supabase.channel as any)
      .mockReturnValueOnce(gamesChannel)
      .mockReturnValueOnce(turnsChannelA)
      .mockReturnValueOnce(turnsChannelB);
...
describe('useGameStore.startGame', () => {
  ...
  it('失败时会抛出错误', async () => {
    ...
    await expect(
      useGameStore.getState().startGame('room-1')
    ).rejects.toBeDefined();
  });
});
