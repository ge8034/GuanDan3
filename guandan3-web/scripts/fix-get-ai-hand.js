...
`;
    ...
    console.log('执行 get_ai_hand 修复...');
    await client.query(sql);
    console.log('✅ 修复成功！get_ai_hand 现在支持练习模式');

    // 验证修复
    const result = await client.query(`
      SELECT routine_name, routine_definition
      FROM information_schema.routines
      WHERE routine_name = 'get_ai_hand'
      AND routine_schema = 'public'
    `);

    if (result.rows.length > 0) {
      console.log('✅ 验证成功：函数已更新');
      console.log('包含 pve1v3 检查:', result.rows[0].routine_definition.includes('pve1v3'));
    }
