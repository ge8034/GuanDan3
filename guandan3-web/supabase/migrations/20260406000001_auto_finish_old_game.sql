-- 自动完成旧游戏后再开始新游戏
-- 修复P0001错误: "A game is already playing in this room"
...
BEGIN
  ...
  UPDATE public.games
  SET state_private = jsonb_build_object(
      'hands', v_state_private_hands
    ),
    updated_at = NOW()
  WHERE id = v_game_id;
...
COMMENT ON FUNCTION public.start_game IS '开始一局新游戏，自动完成旧游戏（修复P0001）';
