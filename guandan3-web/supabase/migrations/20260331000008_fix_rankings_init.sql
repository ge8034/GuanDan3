-- 修复 submit_turn 中 v_rankings 的初始化
-- 确保 v_rankings 始终被初始化，避免返回 NULL 导致类型转换错误
-- 2026-03-31
...
declare
  ...
  v_idx int;
begin
  -- 初始化 rankings 为空数组（使用 ARRAY[] 构造函数）
  -- v_rankings 默认为 NULL，会在使用时初始化
  ...
  -- 10. Return result
  -- 确保 rankings 不为 NULL（Supabase RPC 可能有解析问题）
  return query
    select v_turn_no + 1, (v_current_seat + 1) % 4, v_game_status, coalesce(v_rankings, ARRAY[]::int[]);
end;
$$;
