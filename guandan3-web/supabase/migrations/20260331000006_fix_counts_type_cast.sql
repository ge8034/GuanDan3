-- 修复 submit_turn 中 counts 类型转换问题
-- 问题: jsonb 数组不能直接转换为 int[]
-- 解决: 使用 jsonb_array_elements_text 正确转换
...
begin
  ...
  if v_action_type = 'play' and jsonb_array_length(v_played_cards) > 0 then
    ...
    if jsonb_array_length(v_new_hand) = 0 then
      ...
      if v_state_public ? 'rankings' then
        v_rankings := (v_state_public->'rankings')::int[];
      else
        v_rankings := int[4];
      end if;
      ...
      if array_length(v_rankings, 1) >= 3 then
        -- 对家出完
        v_state_public := jsonb_set(v_state_public, array['status'], to_jsonb('finished'));
        v_state_private := jsonb_set(v_state_private, array['status'], to_jsonb('finished'));
      end if;
    end if;
  end if;
  ...
  -- 10. Return result
  return query
    select v_turn_no + 1, (v_current_seat + 1) % 4, v_state_public->>status, v_rankings;
end;
$$;
