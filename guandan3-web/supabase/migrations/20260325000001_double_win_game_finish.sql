...
begin
  ...
  -- 新规则：双人获胜（头游+二游同队）时游戏立即结束
  if jsonb_array_length(v_rankings) >= 2 then
    -- 检查头游和二游是否同队
    declare
      v_first_ranked int;
      v_second_ranked int;
      ...
      v_second_team int;
    begin
      v_first_ranked := (v_rankings->0)::int;
      v_second_ranked := (v_rankings->1)::int;
      v_first_team := v_first_ranked % 2;
      v_second_team := v_second_ranked % 2;

      -- 如果头游和二游同队，游戏结束
      if v_first_team = v_second_team then
        v_status := 'finished';
      ...
      -- 原规则：3人出完牌时结束
      if jsonb_array_length(v_rankings) >= 3 and v_status <> 'finished' then
        v_status := 'finished';
