-- 修复练习模式（匿名用户）的游戏开始问题
-- 问题：start_game 函数对于匿名用户（auth.uid() 返回 NULL）验证失败
-- 解决：允许练习模式的房间所有者（包括匿名用户）开始游戏
...
BEGIN
  ...
  IF v_room_mode = 'pve1v3' THEN
    ...
    IF NOT EXISTS (
      ...
      RAISE EXCEPTION 'Practice room not ready to start (status must be open)';
    END IF;
  ELSE
    -- PVP 模式：检查所有者权限
    IF NOT EXISTS (
      ...
      WHERE id = p_room_id
      AND status = 'open'
      AND (owner_uid = auth.uid() OR auth.uid() IS NULL AND owner_uid IS NULL)
    ) THEN
      RAISE EXCEPTION 'Room not found or not ready to start (must be owner)';
    END IF;
  END IF;
  ...
  -- 5. 生成牌组 (两副牌，每副52张 = 104张 + 4张大小王 = 108张)
  v_deck := ARRAY[]::TEXT[];
...
-- 添加注释
COMMENT ON FUNCTION start_game IS '开始一局新游戏，支持练习模式（匿名用户）和PVP模式';
