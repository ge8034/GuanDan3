-- 修复 submit_turn 函数以正确处理 AI 玩家
-- 问题：AI 玩家没有 uid，不能在 game_hands 表中创建记录
-- 解决：只为人类玩家更新 game_hands，AI 只更新 state_private.hands

-- 先删除现有函数（因为返回类型需要改变）
DROP FUNCTION IF EXISTS public.submit_turn(uuid, uuid, int, jsonb);

-- 创建新的 submit_turn 函数，支持 AI 移动并正确更新手牌
...
-- 添加注释
COMMENT ON FUNCTION public.submit_turn IS '提交玩家出牌动作，支持AI移动，正确更新 state_private.hands 和 game_hands（仅人类玩家）';
