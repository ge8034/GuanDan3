-- 修复 submit_turn 中 v_new_counts 的类型
-- 将 int[4] 改为 int[]，并使用不同的数组初始化方式
-- 2026-03-31
...
begin
  ...
  if v_action_type = 'play' and jsonb_array_length(v_played_cards) > 0 then
    ...
    -- 更新 counts - 使用简化方式避免类型转换问题
    -- 直接从 state_public 获取并修改
    if v_state_public ? 'counts' then
      -- 提取现有 counts，按位置顺序（使用 with ordinality）
      select array_agg(elem order by idx)
      into v_new_counts
      from jsonb_array_elements(v_state_public->'counts') with ordinality as t(elem, idx);
    else
      v_new_counts := ARRAY[27, 27, 27, 27];
    end if;

    -- 更新当前玩家的手牌数量
    v_new_counts[v_current_seat + 1] := jsonb_array_length(v_new_hand);

    -- 转换为 jsonb 并更新 state_public
    v_state_public := jsonb_set(
      coalesce(v_state_public, '{}'::jsonb),
      array['counts'],
      ('[' || array_to_string(v_new_counts, ',') || ']')::jsonb
    );
