-- 修复 submit_turn 函数以正确更新 state_private.hands
-- 问题：submit_turn 只更新了 turn_no 和 current_seat，但没有从 state_private.hands 中移除已出的牌
-- 解决：在 submit_turn 中更新 state_private.hands，移除玩家出的牌

-- 先备份现有的 submit_turn 函数
CREATE OR REPLACE FUNCTION submit_turn_backup(
  p_game_id uuid,
  p_action_id uuid,
...
-- 创建新的 submit_turn 函数，正确更新 state_private.hands
...
-- 添加注释
COMMENT ON FUNCTION public.submit_turn IS '提交玩家出牌动作，正确更新 state_private.hands 和 state_public';
