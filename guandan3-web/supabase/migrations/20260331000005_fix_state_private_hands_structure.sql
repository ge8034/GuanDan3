-- 修复 start_game 中的 state_private.hands 结构
-- 问题：双重嵌套导致 {"hands": {"0": [...], ...}} 而不是 {"0": [...], ...}
-- 解决：直接使用 v_state_private_hands 而不是再包装一层
...
BEGIN
  ...
  -- 构建 state_private.hands JSON (直接构建，不需要额外的 'hands' 包装)
  ...
  -- 修复：直接使用 v_state_private_hands 作为 state_private，而不是包装在 'hands' 键中
  ...
  VALUES (
    ...
    v_state_private_hands  -- 直接使用，不再包装
  )
...
COMMENT ON FUNCTION public.start_game IS '开始一局新游戏，正确初始化 state_private.hands 结构';
