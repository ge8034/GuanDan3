...
本文档定义了 GuanDan3 项目的后端架构开发标准，旨在解决项目中发现的根本性架构问题，确保系统的高性能、可扩展性、安全性和可维护性。

## 核心问题解决方案

### 问题1：异步依赖混乱
**问题描述**: `initGame`异步但没有await，清理函数过早设置，可能导致执行多次和资源泄漏。
...
### 问题2：事件监听器无限循环
**问题描述**: `setupEventListeners`递归调用，没有可靠的停止条件，可能导致死循环。
  ...
  setupEventListeners(container: HTMLElement): void {
    ...
    if (!container || !container.isConnected) {
      if (this.retryCount < this.maxRetries) {
        this.retryCount++;
        setTimeout(() => this.setupEventListeners(container), 100);
        return;
      }
      throw new Error('Failed to setup event listeners: container not ready');
    }
...
### 问题3：缺少统一的初始化管理
**问题描述**: 游戏规则引擎初始化不完整，导致功能缺失和状态不一致。

**解决方案**:
```typescript
// 统一的初始化管理框架
export class GameEngineInitializer {
  private static instance: GameEngineInitializer;
  private initializationPromise: Promise<void> | null = null;
  private dependencies = new Map<string, any>();

  static getInstance(): GameEngineInitializer {
    if (!GameEngineInitializer.instance) {
      GameEngineInitializer.instance = new GameEngineInitializer();
    }
    return GameEngineInitializer.instance;
  }
  ...
  async initialize(): Promise<void> {
    ...
    this.initializationPromise = (async () => {
      try {
        ...
        // 2. 初始化游戏规则引擎
        await this.initializeGameRules();
  ...
  private async initializeGameRules(): Promise<void> {
    // 确保游戏规则只初始化一次
    if (this.dependencies.has('gameRules')) {
      return;
    }

    const gameRules = await import('@/lib/game/rules');
    // 初始化游戏规则
    await gameRules.initialize();

    this.dependencies.set('gameRules', gameRules);
  }
...
// 使用示例
export async function ensureGameEngineInitialized(): Promise<void> {
  const initializer = GameEngineInitializer.getInstance();
